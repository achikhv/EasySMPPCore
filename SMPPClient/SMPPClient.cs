...
namespace EasySMPP
{
    ...
    public class SMPPClient
    {
        ...
        public int SendSms(String from, String to, bool splitLongText, String text, byte askDeliveryReceipt, byte esmClass, byte dataCoding)
        {
            ...
            sourceAddressTon = smscArray.currentSMSC.SourceTon != null ? (byte)smscArray.currentSMSC.SourceTon : getAddrTon(sourceAddress);
            sourceAddressNpi = smscArray.currentSMSC.SourceNpi != null ? (byte)smscArray.currentSMSC.SourceNpi : getAddrNpi(sourceAddress);

            destinationAddress = Tools.GetString(to, 20, "");
            destinationAddressTon = smscArray.currentSMSC.AddrTon != null ? (byte)smscArray.currentSMSC.AddrTon : getAddrTon(destinationAddress);
            destinationAddressNpi = smscArray.currentSMSC.AddrNpi != null ? (byte)smscArray.currentSMSC.AddrNpi : getAddrNpi(destinationAddress);
        ...
        private void connectToSMSC()
        {
            try
            {
                ...
                IPAddress ipAddress;
                bool ip = IPAddress.TryParse(smscArray.currentSMSC.Host, out ipAddress);
        ...
        private void bind()
        {
            try
            {
                ...
                pos = 7;
                Bind_PDU[pos] = (byte)smscArray.currentSMSC.BindMode;
                            ...
                            if ((_command_length <= (mLen - x)) && (_command_length >= 16))
                            {
                                ...
                                switch (_command_id)
                                {
                                    ...
                                    case 0x80000009: // bind_transceiver_resp
                                        logMessage(LogLevels.LogSteps, "Bind Resp");
